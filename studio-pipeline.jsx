...
function AuthScreen({ onLogin }) {
  ...
  const handleSubmit = async () => {
    ...
    if (provider === "email" && !form.password.trim()) { setError("Please enter a password."); return; }
    setLoading(true); setError("");
    try {
      const res = await window.storage.get("studio_users");
      const allUsers = res ? JSON.parse(res.value) : {};
      let user = allUsers[email];
      if (user) {
        if (provider === "email" && user.password !== form.password) {
          setError("Incorrect password. Try again."); setLoading(false); return;
        }
      } else {
        if (!form.name.trim()) { setError("Please enter your name to create an account."); setLoading(false); return; }
        user = { id:uid(), name:form.name.trim(), email, provider:provider||"email",
                 password: provider==="email" ? form.password : "", createdAt:new Date().toISOString() };
        allUsers[email] = user;
        await window.storage.set("studio_users", JSON.stringify(allUsers));
      }
      await window.storage.set("studio_session", JSON.stringify({ email }));
      setLoading(false);
      onLogin(user);
    } catch(e) {
  ...
  const providerIcon  = { google:"G", apple:"", email:"✉" };
      ...
      <div className="auth-card">
        ...
        {!provider ? (
          <>
            ...
            {["google","apple"].map(p => (
              <button key={p} className="social-btn" onClick={() => setProvider(p)}>
          ...
          <>
            <button onClick={() => { setProvider(null); setError(""); }}
            ...
            <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:24,padding:"12px 16px",background:"var(--cream)",borderRadius:10 }}>
              <span style={{ fontSize:13,color:"var(--soft)" }}>
                {provider==="google" ? "Signing in with Google" : provider==="apple" ? "Signing in with Apple" : "Email sign-in"}
              </span>
            </div>
            ...
            <Field label="Email Address">
              <input type="email" value={form.email} onChange={e => sf("email",e.target.value)}
                placeholder="you@example.com" onKeyDown={e => { if(e.key==="Enter") handleSubmit(); }} />
            </Field>
            {provider === "email" && (
              <Field label="Password">
                <input type="password" value={form.password} onChange={e => sf("password",e.target.value)}
                  placeholder="••••••••" onKeyDown={e => { if(e.key==="Enter") handleSubmit(); }} />
              </Field>
            )}
            {error && (
            ...
            <p style={{ fontSize:11,color:"var(--soft)",textAlign:"center",marginTop:14 }}>
      ...
      <div className="modal modal-wide">
        ...
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6 }}>
          <div>
            ...
            <h2 style={{ fontFamily:"var(--serif)",fontSize:26,fontWeight:400 }}>Edit Default Template</h2>
        ...
        <div style={{ maxHeight:"52vh",overflowY:"auto",paddingRight:4 }}>
          <PhasesPanel phases={local} checks={null} onPhasesChange={setLocal} onChecksChange={null} alwaysEdit={true} />
        </div>
...
// ── Docs & Links Tab ──────────────────────────────────────────────────────────
function DocsLinksTab({ files, links, setFiles, setLinks }) {
  ...
  const processFiles = list => {
    ...
    Promise.all(valid.map(file => new Promise(res => {
      ...
      r.onload = e => res({ id:uid(), name:file.name, size:file.size, type:file.type, dataUrl:e.target.result, addedAt:new Date().toISOString() });
      r.readAsDataURL(file);
    }))).then(nf => setFiles([...files, ...nf]));
  };
  ...
  const downloadFile = f => { const a=document.createElement("a"); a.href=f.dataUrl; a.download=f.name; a.click(); };
          ...
          <div style={{ background:"var(--white)",border:"1px solid var(--border)",borderRadius:10,overflow:"hidden" }}>
            {files.map((f,i) => (
              <div key={f.id} className="file-row" style={{ borderBottom:i<files.length-1?"1px solid var(--cream)":"none" }}>
                ...
                <button className="btn btn-ghost" style={{ padding:"5px 12px",fontSize:11,flexShrink:0 }} onClick={() => downloadFile(f)}>⬇ Save</button>
...
// ── Project Editor ────────────────────────────────────────────────────────────
function ProjectEditor({ project: initProject, user, onUpdate, onClose, template, onSaveTemplate }) {
  ...
  return (
    <>
      ...
      {tplEditorOpen && <TemplateEditorModal template={template} onSave={onSaveTemplate} onClose={() => setTplEditorOpen(false)} />}
      ...
      <div style={{ minHeight:"100vh",background:"var(--paper)" }}>
        ...
        <div style={{ display:"flex",maxWidth:1100,margin:"0 auto",padding:"32px 24px",gap:28,alignItems:"flex-start" }}>
          ...
          <main style={{ flex:1,minWidth:0 }}>
            ...
            {activeTab==="docs" && (
              <DocsLinksTab files={project.files||[]} links={project.links||[]} setFiles={setFiles} setLinks={setLinks} />
            )}
...
export default function App() {
  ...
  const loadSession = async () => {
    try {
      const sRes = await window.storage.get("studio_session");
      if (sRes) {
        const { email } = JSON.parse(sRes.value);
        const uRes = await window.storage.get("studio_users");
        if (uRes) {
          const users = JSON.parse(uRes.value);
  ...
  const loadAll = async () => {
    try { const r=await window.storage.get("studio_all_projects"); if(r) setProjects(JSON.parse(r.value)); } catch(e) {}
    try { const r=await window.storage.get("studio_cl_template"); if(r) setTemplate(JSON.parse(r.value)); } catch(e) {}
  };

  const saveProjects = async updated => {
    setProjects(updated);
    try {
      const forStorage = updated.map(p => ({ ...p, files:(p.files||[]).map(({dataUrl,...rest})=>rest) }));
      await window.storage.set("studio_all_projects", JSON.stringify(forStorage));
    } catch(e) {}
  };

  const saveTemplate = async tpl => {
    setTemplate(tpl);
    try { await window.storage.set("studio_cl_template", JSON.stringify(tpl)); } catch(e) {}
  };

  const handleLogin = async user => { setCurrentUser(user); await loadAll(); };
  const handleLogout = async () => {
    try { await window.storage.delete("studio_session"); } catch(e) {}
  ...
  const updateProject = updated => {
    ...
    saveProjects(projects.map(p => p.id===updated.id ? { ...updated,updatedAt:now } : p));
  };
  ...
  const duplicateProject = id => {
    ...
    if (!orig) return;
    const dup = { ...clone(orig), id:uid(), title:`${orig.title} (Copy)`, ownerId:currentUser.id, ownerName:currentUser.name, ownerEmail:currentUser.email, collaborators:[], createdAt:new Date().toISOString(), updatedAt:new Date().toISOString(), files:[] };
    saveProjects([...projects, dup]);
  };
